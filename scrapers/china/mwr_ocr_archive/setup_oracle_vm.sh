#!/usr/bin/env bash
echo "Retired setup: use the GitHub Actions MWR API collector instead." >&2
exit 1
...
sudo -u "$TARGET_USER" "$VENV_DIR/bin/pip" install -r "$REPO_DIR/scrapers/china/mwr_ocr_archive/requirements.txt"

echo "==> Done."
echo
echo "Next steps:"
echo "  1. bash $REPO_DIR/scrapers/china/mwr_ocr_archive/setup_git_deploy_key.sh"
echo "  2. Test: OUTPUT_DIR=$REPO_DIR/data/china/mwr_ocr_archive \\"
echo "           $VENV_DIR/bin/python $REPO_DIR/scrapers/china/mwr_ocr_archive/grab_data.py"
echo "  3. Add cron entry (see scrapers/china/mwr_ocr_archive/README.md)."
