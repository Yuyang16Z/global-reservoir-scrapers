# MWR OCR 部署到 Oracle Cloud Free Tier（已停用）

> 此部署路径已于 2026-07-24 停用，仅保留作历史审计。请勿重新配置
> cron；当前采集统一使用 `scrapers/china/mwr_api/`。
...
# 装系统包 + venv + Python 依赖
sudo bash scrapers/china/mwr_ocr_archive/setup_oracle_vm.sh
```
...
```bash
cd ~/global-reservoir-scrapers
bash scrapers/china/mwr_ocr_archive/setup_git_deploy_key.sh
```
...
```bash
cd ~/global-reservoir-scrapers
OUTPUT_DIR="$PWD/data/china/mwr_ocr_archive" \
MWR_HEADLESS=1 \
~/venvs/mwr/bin/python scrapers/china/mwr_ocr_archive/grab_data.py
```
...
```bash
ls data/china/mwr_ocr_archive/$(date -u +%F)/
...
```cron
0 12 * * * /bin/bash -lc '$HOME/global-reservoir-scrapers/scrapers/china/mwr_ocr_archive/run_and_commit.sh'
```
...
等 VM 上 cron 跑通一次之后（明天 20:00 看 `data/china/mwr_ocr_archive/run_logs/` 有没有新文件），在 Mac 上：
...
- **cron 跑但不提交**：看 `data/china/mwr_ocr_archive/run_logs/cron_*.log`
